...
st.set_page_config(
    page_title="FPD Daily - Dashboard Pro",
    layout="wide",
    page_icon="📊"
)

# Diccionario para nombres de meses dinámicos
MESES_NOMBRE = {
...
def get_filter_universes():
    """Obtiene valores únicos para los filtros del sidebar."""
    con = duckdb.connect()
    return con.execute("""
        SELECT DISTINCT 
...
def get_main_data(regionales, sucursales, productos, tipos):
    """Datos para Tab 1 y Tab 3. Si las listas están vacías, no filtra (selecciona todos)."""
...
def get_executive_data(field):
    """Datos para Tab 2. Independiente de filtros, excluye Nóminas y Sucursal 999."""
    query = f"""
    WITH base AS (
        SELECT 
            TRY_CAST(strptime(fecha_apertura, '%d/%m/%Y') AS DATE) as fecha_dt,
    ...
    SELECT strftime(fecha_dt, '%Y%m') as cosecha_id, RIGHT(strftime(fecha_dt, '%Y%m'), 2) as mes_id,
           dimension, COUNT(id_credito) as total_vol, SUM(fpd2_num) as fpd2_si,
...
st.sidebar.markdown("*(Si no seleccionas nada, considera todo)*")
...
with tabs[0]:
    if not df_main.empty:
        # Agregación para KPIs y Gráficas
        ...
        with c1:
            ...
            fig1 = go.Figure(go.Scatter(x=df_t['cosecha_id'], y=df_t['rate'], mode='lines+markers+text', text=df_t['rate'].apply(lambda x: f'{x:.1f}%'), line=dict(color='#1B4F72', width=4), fill='tozeroy', fillcolor='rgba(27,79,114,0.1)', name='Global'))
        ...
        with c2:
            ...
            fig2 = px.line(df_o, x='cosecha_id', y='rate', color='origen2', markers=True, color_discrete_map={'fisico':'#2E86C1','digital':'#CB4335'})
            fig2.update_layout(xaxis=dict(type='category'), plot_bgcolor='white', height=350, legend=LEGEND_BOTTOM)
            st.plotly_chart(fig2, use_container_width=True)

        # Fila 2: Interanual y FPD vs NP
        c3, c4 = st.columns(2)
        with c3:
        ...
        with c4:
            ...
            fig4 = go.Figure()
            fig4.add_trace(go.Scatter(x=df_t['cosecha_id'], y=df_t['rate'], name='% FPD2', line=dict(color='#1B4F72')))
...
with tabs[1]:
    ...
    def render_exec_section(field, title, dim_label):
        ...
        if not df_e.empty:
            ...
            mes_u = MESES_NOMBRE.get(ult_c[-2:], 'N/A'); mes_a = MESES_NOMBRE.get(ant_c[-2:], 'N/A')
            ...
            st.subheader(f"📋 Ranking {title} - Cosecha {ult_c}")
    ...
    render_exec_section('unidad_regional', "Regional", "Regional")
    render_exec_section('producto_agrupado', "por Producto", "Producto")
    render_exec_section('sucursal', "por Sucursal", "Sucursal")
...
with tabs[2]:
    if not df_main.empty:
        ...
        # 1. Heatmap Regional
        st.subheader("📍 Mapa de Riesgo Regional (Tendencia 6 meses)")
        ...
        # 2. Pareto Sucursales
        st.subheader("🏢 Pareto de Sucursales (Concentración de Casos)")
        ...
        fig_p = go.Figure([go.Bar(x=df_p['sucursal'].head(20), y=df_p['fpd2_num'].head(20), name="Casos"),
                           go.Scatter(x=df_p['sucursal'].head(20), y=df_p['acum'].head(20), name="% Acumulado", yaxis="y2", line=dict(color='red'))])
        fig_p.update_layout(yaxis2=dict(overlaying="y", side="right", range=[0,110], title="% Acumulado"), plot_bgcolor='white', legend=LEGEND_BOTTOM, height=500)
        st.plotly_chart(fig_p, use_container_width=True)

        # 3. Sensibilidad Monto
        st.subheader("💰 Riesgo por Monto Otorgado (Buckets $5k)")
        df_main['bucket'] = (df_main['monto_otorgado'] // 5000) * 5000
        df_s = df_main.groupby('bucket').agg({'fpd2_num':'sum','id_credito':'count'}).reset_index()
        df_s['rate'] = (df_s['fpd2_num']*100/df_s['id_credito'])
        df_s['label'] = df_s['bucket'].apply(lambda x: f"${x:,.0f}-${x+5000:,.0f}")
        fig_s = px.bar(df_s, x='label', y='rate', color='rate', color_continuous_scale='Reds', text=df_s['rate'].apply(lambda x: f'{x:.1f}%'))
        fig_s.update_layout(plot_bgcolor='white', coloraxis_showscale=False, height=450)
